function Slider(target, type, per = 1) {
    ...
    const isNumbering = true; // 번호 추가
    const numberingName = 'current-number-area';
    ...
    function moveSlide(index) {
        ...
        if (isNumbering) {
            slider.querySelector('.current-number-area').innerText = index + ' / ' + (size - 2);
        }
    }
    ...
    if (isNumbering) {
        ...
        number.classList.add(numberingName);
        number.innerText = index + ' / ' + (size - 2);
